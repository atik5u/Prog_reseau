...
public class ThreadGame extends Thread{
	...
	public ThreadGame(Socket joueur, Game game){
		try {
			...
			System.out.println("Erreur ThreadGame");
			e.printStackTrace();
		}
	}
		...
		while (i < ships.size()){
			...
			try {
				if (checkPositionFormat(choice)) {
					this.out.println("Mauvaise format de la position du navire.");
				...
				this.out.println("Voulez-vous reéntrer la position au format suivante: x,y,o (ex: a,1,v)");
				this.out.println("Vérifiez que la distance entre deux navires est au moins un carré.");
			}
		}
		game.ready();
		out.println("En attendant d'l adversaire.");
		
		while(game.getReadyPlayers() != 2) {
			try {
				Thread.sleep(500);
			} catch (InterruptedException e) {
				System.out.println("Error SLEEP enter Position");
				e.printStackTrace();
			}
		}
	}
	...
	public void run() {
		try {
			...
			System.out.println("Problème ThreadGame " + playerNum + " run()");
			e.printStackTrace();
		}
	}
}
